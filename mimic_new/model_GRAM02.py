...
def generate_attention(tparams, leaves, ancestors):
    ...
    mlpOutput = V(tf.tanh(tf.matmul(attentionInput, tparams['W_attention']) + tparams['b_attention']))
...
if __name__ == '__main__':
    ...
    model = keras.models.Sequential([
        ...
        keras.layers.SimpleRNN(128, return_sequences=True, dropout=0.5),
        keras.layers.Dense(283, activation='softmax')
    ])

    # attention_input = keras.layers.Input((x.shape[1], x.shape[2]), name='gru_input')
    ...
    model.summary()
    # checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='G:\\模型训练保存\\gram_05', monitor='val_accuracy', mode='auto',
    #                                                 save_best_only='True')
    #
