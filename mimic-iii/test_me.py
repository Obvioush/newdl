...
class KnowledgeAttention(keras.Model):
    ...
    def call(self, Lt, rnn_ht):
        # Lt.shape:(batch_size, length, units)=>(5250,41,128)
        # rnn_ht.shape:(batch_size, length, units) =>(5250,41,128)

        # score.shape:(batch_size, length, 1)
        score = self.V(tf.nn.tanh(
                self.W1(rnn_ht) + self.W2(Lt)))

        # (Alpha)attention_weights.shape:(batch_size, length, 1)
        attention_weights = tf.nn.softmax(score, axis=1)

        # context_vector.shape: (batch_size, length, units)
        # tensorflow自己做扩展，把Alpha的1扩展为units
        context_vector = attention_weights * rnn_ht

        # 在length维度求和，context_vector.shape:(batch_size, length, units)
        context_vector = tf.reduce_sum(context_vector, axis=1)
