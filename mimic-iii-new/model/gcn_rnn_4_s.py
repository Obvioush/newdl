...
def padMatrix(seqs, labels, treeseqs=''):
    ...
    tree = np.zeros((n_samples, maxlen, treeCount)).astype(np.float32)
...
class metricsHistory(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        pred = model.predict([x_test, tree_test])
...
if __name__ == '__main__':
    ...
    tree_emb = keras.layers.Dense(128, kernel_initializer=keras.initializers.constant(knowledge_emb),
                                    trainable=False, name='tree_emb')(tree_mask)
    ...
    model_output = keras.layers.Dense(labelCount, activation='softmax', name='main_output')(st[:,-1,:])
