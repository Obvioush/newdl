...
def padMatrix1(seqs, labels, treeseqs):
    ...
    inputDimSize = calculate_dimSize('./resource/process_data/process.dataseqs')
    numClass = calculate_dimSize('./resource/process_data/process.labelseqs')
    treeDimSize = calculate_dimSize('./resource/process_data/process.treeseqs')
...
if __name__ == '__main__':
    ...
    treeFile = './resource/process_data/process.treeseqs'
    ...
    inputs = tf.keras.Input(shape=(x.shape[1], x.shape[2]), name="inputs")
    # masks = tf.keras.Input(shape=(x.shape[1], x.shape[2]), name='masks')
    # 添加一个Masking层，这个层的input_shape=(timesteps, features)
    mask = keras.layers.Masking(mask_value=0, input_shape=(x.shape[1], x.shape[2]))(inputs)
    # encodings = PositionEncoding(x.shape[2])(mask)
    # encodings = Add()([mask, encodings])
    # s = MultiHeadAttention(1, 512)([encodings, encodings, encodings])
    s = MultiHeadAttention(1, 512)([mask, mask, mask])
    # s = keras.layers.Dropout(0.2)(s)
    # s = keras.layers.Dense(512, activation='relu')(s)
    outputs = keras.layers.Dense(283, activation='softmax')(s)
