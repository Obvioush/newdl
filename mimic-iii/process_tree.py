...
def process_trees(dataseqs, tree_old):
    leaf2tree = pickle.load(open('./resource/trees.level5.pk', 'rb'))
    trees_l4 = pickle.load(open('./resource/trees.level4.pk', 'rb'))
    trees_l3 = pickle.load(open('./resource/trees.level3.pk', 'rb'))
    trees_l2 = pickle.load(open('./resource/trees.level2.pk', 'rb'))
    tree_seq = []
...
if __name__ == '__main__':
    ...
    trees_type = pickle.load(open('./resource/node2vec_edgelist.oldtypes', 'rb'))
    ...
    # 共有728个祖先节点
    for i in range(4894, len(retype)):
    ...
    # pickle.dump(newTreeSeq, open(outFile + '_new.treeseqs', 'wb'), -1)
